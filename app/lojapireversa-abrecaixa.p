...
def var pnomerecurso as char    init "reversa-abrecaixa".

def input  param pestabOrigem as int.
def input  param petbdest     as int.
def output param pcodCaixa  as int.
...
vsaida  = vwork + replace(pnomeapi," ","") + replace(pnomerecurso," ","") +
    ...
    vapi = "http://\{IP\}/bsweb/api/lojas/\{codigoFilial\}/reversa-abrecaixa".
...
then do:
    ...
    then do:
        ...
        find first ttnovacaixa no-error.
        if avail ttnovacaixa
        then do:
            pcodCaixa = int(ttnovacaixa.codcaixa).
        end. 
