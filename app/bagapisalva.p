...
def var pnomerecurso as char    init "salva".
...
def temp-table ttsaida  no-undo serialize-name "conteudoSaida" 
    ...
    ttentrada.cpf           = pcpf.
    ttentrada.nome          = ttbag.nome.
    ttentrada.pid           = spid.
    ttentrada.consultor     = ttbag.consultor.
    ttentrada.categoria     = pcatcod.
    ttentrada.datacriacao   = ttbag.datacriacao.
...
vsaida  = vwork + replace(pnomeapi," ","") + replace(pnomerecurso," ","") +
    ...
    vapi = "http://\{IP\}/bsweb/api/lojas/\{codigoFilial\}/bag-salva/\{cpf\}".
