...
def var pnomerecurso as char    init "verificabloqproduto".

def input   param pprocod     as int.
def output  param pemOperacao as log.
...
if vchost = ""
then do:
    message "lojapi-verificabloqdesconto ERRO api-host ".
    return.
end.
...
def var vapi as char.
    vapi = "http://\{IP\}/bsweb/api/pdv/verificaBloqProduto/\{codigoFilial\}".
...
then do:
    ...
    then do:
        ...
        if avail ttretorno
        then do:
            pemoperacao = ttretorno.produtobloqueio = "true".           
        end. 
