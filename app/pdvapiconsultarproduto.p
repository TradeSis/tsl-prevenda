...
def stream log.
output stream log to value(vlog + "/apipdv_consultarProduto" + string(today,"99999999") + ".log").
...
find apipdvconsultarproduto where 
        apipdvconsultarproduto.etbcod = petbcod and
        apipdvconsultarproduto.procod = pprocod 
        no-lock no-error.
if not avail apipdvconsultarproduto
then do on error undo:
    create apipdvconsultarproduto.
    apipdvconsultarproduto.etbcod = petbcod. 
    apipdvconsultarproduto.procod = pprocod.
...
def var vresposta as char.
    ...
    vapi = replace(vapi,"\{IP\}",vcbarramento).
    vapi = replace(vapi,"\{codigoLoja\}",string(petbcod)).
    vapi = replace(vapi,"\{codigoProduto\}",string(pprocod)).

vsaida  = vwork + "/pdvapiconsultarproduto" + string(today,"999999") + replace(string(time,"HH:MM:SS"),":","") + ".json". 
