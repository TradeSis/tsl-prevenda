/* #112022 helio Venda O, que permite a venda para uma loja que está fechada. */
def new global shared var spid as int.
def new global shared var spidseq as int.

def var pnomeapi as char        init "pdv".
def var pnomerecurso as char    init "verificapromqtd".

def input   param ppromoc     as char.
def input   param pprocod     as int.
def input   param pqtdSolicitada as int.
def output  param ppromocaoLimite as char.
...
def temp-table ttretorno serialize-name "return" 
    ...
    create ttentrada.
    ttentrada.sequenciaCampanha = ppromoc.
    ttentrada.codigoFilial = string(setbcod).
    ttentrada.codigoProduto = string(pprocod).
    ttentrada.qtdSolicitada = string(pqtdSolicitada).
...
if vchost = ""
then do:
    message "lojapi-verificapromqtd ERRO api-host ".
    return.
end.
...
def var vapi as char.
    ...
    vapi = "http://\{IP\}/bsweb/api/pdv/verificaPromQtd/\{codigoFilial\}".
...
then do:
    ...
    then do:
        ...
        if avail ttretorno
        then do:
            ppromocaoLimite = ttretorno.promocaoLimite.
        end. 
